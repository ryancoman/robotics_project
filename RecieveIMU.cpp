...
class ReceiveIMUValues : MonoBehaviour {

    bool UsingWindows = false;
    GameObject endOfClub;
    ...
    string imuName = "r"; // You should ignore this if there is one IMU.
    TrailRenderer tr;
    ...
    Text recordingText;
    TextMeshProUGUI SwingSpeed, ClubAngleParallel, ShaftAngleParallel, ClubAngle, ShaftAngle;
    float maxSpeedval;
    string maxSpeed = "";
    ...
    static float maxGreenZone = 5;
    static float minGreenZone = -5;
    // @jacob - it doesn't like this below - see console warnings
    GetMetrics metricsInstance = new GetMetrics();
    ...
    BluetoothGattCharacteristic characteristic; //this is used for BLE
    string selector = GattDeviceService.GetDeviceSelectorFromUuid(Guid.Empty);
    void Start() {
    ...
    string ReadFromArduino(int timeout = 1000)
    {
